...
```
点检查更新
  ├─ dev 构建 → 调试模式下强制走"可更新"链路（用于联调检查更新 + 下载流程）
  └─ release 构建 → 调 Rust command → fetch 官网 API
       ├─ API 返回错误 / 网络异常 → toast：失败原因（保留按钮可重试）
       ├─ latest <= current → inline："当前已是最新版本 vX.Y.Z"
       └─ latest > current → 关于页顶部出现「更新状态栏」
              - 文案示例："发现新版本 vX.Y.Z，可立即下载"
              - 操作：立即下载 / 取消下载 / 重试下载
              - 样式：与其他连接页顶部提示条保持同一视觉语言（surface-2 + 1px border + 左侧 icon）
              - 交互：状态栏只在 about 页顶部占位，不使用全屏弹层，不阻断用户继续浏览和使用应用

                 用户点「立即下载」后，触发 start_update_download（Rust 后台任务 + 实时事件）：
                   ① 分块下载写入 ~/Downloads/{filename}.dmg，每块 emit 下载状态（进度/字节）
                   ② 下载中可点「取消下载」，取消后保留「重试下载」入口
                   ③ 若下载失败，或长时间未开始传输（长时间 downloaded=0），顶部状态栏统一切换为"官网下载引导"（不展示进度条）
                   ④ 下载完成后切换「正在打开安装包…」→ 调系统 `open <dmg>` 挂载并打开 Finder 卷 → app.exit(0)
```

补充约束：
- 下载状态栏为**页面内非模态**提示，不允许出现"确认后无法关闭"的阻塞体验。
- 下载过程不影响侧边栏切换和其他功能继续使用。
- 下载失败与"长期未开始下载"场景，提示策略统一：引导用户前往官网重新下载并覆盖安装。
- 顶部提示栏文案尽量简洁，默认单行展示；避免多行换行。
- 下载失败与"长期未开始下载"状态下，不展示进度条。
...
- **dev 构建默认用于联调更新链路**：检查更新会优先复用官网版本信息并强制判定有更新（仅 debug 生效），便于反复验证下载、取消、失败、重试等流程；release 构建不受影响
