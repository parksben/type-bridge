...
- Footer tagline：「聊天即打字 · macOS 桌面应用」
...
下载卡下方的"首次安装须知"说明唯一可靠的解决方式：

- **CLI 路径**：将应用拖入程序坞后，在终端执行 `xattr -rd com.apple.quarantine /Applications/TypeBridge.app`，命令块必须带"复制"按钮

> 说明：未公证应用在 macOS「系统设置 → 隐私与安全性」中通常不会出现被拦截的条目，GUI 路径（「仍要打开」）不可靠，故不再提供该引导。

卡片最后附"首次使用仍需授予辅助功能权限"提示。

### 9.5 场景文案

4 个场景 pill tab 轮播（`#scenes`），5s 自动切换，hover 暂停：

1. **手机屏幕，就是触控板** — 单指移鼠标，双指滚页面，三指切桌面。不用蓝牙配对，扫码即用
2. **手机上打字，电脑上出字** — 光标在哪字就落在哪，任何输入框都行。支持自动提交（发送即回车）
3. **张嘴说，电脑写** — 手机输入法语音转文字，说完点发送，文字直达电脑光标处
4. **快捷指令，一触即发** — 截屏(Cmd+Shift+4)、全选(Cmd+A)、复制粘贴(Cmd+C/V)、撤销(Cmd+Z)、方向键、翻页等高频快捷键，手机上点一下，电脑即刻响应。手不用碰键盘
