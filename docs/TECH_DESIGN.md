...
│10. 收集 .dmg 产物                   │
│11. 创建/更新 GitHub Release         │
│12. 上传 .dmg assets 到 Release      │
└─────────────────────────────────────┘
```
...
| `/download/[arch]` | Route Handler | 动态查 GitHub API，流式透传 |
...
（保持现有设计）
- `GET /download/[arch]` Route Handler → fetch GitHub Release 最新资产 → 流式透传
- Netlify CDN 边缘节点提供对中国用户的下载加速
...
│   ├── download/[arch]/route.ts  # Route Handler 透传 GitHub Release .dmg
