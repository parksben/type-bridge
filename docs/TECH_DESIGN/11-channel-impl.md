...
### 32.7 连接态操作布局优化（飞书 / 钉钉 / 企微）

连接页按钮分层调整为：

- 主操作（同一行，仅 2 个）：`重启长连接` + `断开连接`
- 次级操作：`测试连接`移动到连接状态行右侧，改为低权重按钮（文本/细边框）

这样可避免连接后按钮从 2 个突变 3 个造成视觉跳动，同时让高频路径（启动/重启/断连）更聚焦。

### 32.8 断连命令语义（stop_channel）

`stop_channel(channel)` 语义升级为**真正断连**：

1. 从 `SidecarBridge` 中取出该渠道 `CommandChild`
2. 立即调用 `kill()` 终止 sidecar 进程
3. 主动 emit `typebridge://status { connected:false }`，前端无需等待进程退出事件

说明：`start_*` 仍保持原语义（若该渠道已连接则先 stop 再 start），因此「重启长连接」仍复用原按钮逻辑。

### 32.9 断连确认弹窗文案（动态化）

断连按钮点击后，`window.confirm` 文案按当前子 tab 渠道动态渲染，模板统一为三句：

1. 确认动作：确认断开 `{channelLabel}` 长连接吗？
2. 影响提示：断开后将停止接收该渠道新消息
3. 恢复提示：如需继续使用，点击「启动长连接」重新连接

文案要求：短句、直观、避免术语堆叠；三渠道仅替换渠道名，不分叉维护三份文案。
