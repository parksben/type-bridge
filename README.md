...
典型场景：手机上用飞书发一段语音（转文字），桌面端同步将文本写入正在用的编辑器 / 终端 / 浏览器输入框，实现"语音驱动桌面输入"。
...
```bash
# 生成 .dmg（产物在 src-tauri/target/release/bundle/dmg/）
npm run tauri build
```

默认只编 `aarch64-apple-darwin`。如需 Intel：

1. 额外编译 `feishu-bridge-x86_64-apple-darwin` 二进制
2. 调整 `src-tauri/tauri.conf.json` 的 `bundle.targets`
3. 使用 `npm run tauri build -- --target universal-apple-darwin` 打 universal 包
...
- **先文档后代码**：任何改动先更 [docs/REQUIREMENTS.md](docs/REQUIREMENTS.md) / [docs/TECH_DESIGN.md](docs/TECH_DESIGN.md)，再动代码
